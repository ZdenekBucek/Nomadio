...
export default async function ItineraryPage({ params, searchParams }: Props) {
  ...
  const placeMessages = { created:"Místo bylo uloženo.",updated:"Místo bylo upraveno.",removed:"Místo bylo odstraněno.","mapbox-saved":"Místo z Mapboxu bylo uloženo.","mapbox-invalid":"Vybraný výsledek není platný.","mapbox-error":"Místo z Mapboxu se nepodařilo uložit.","geoapify-saved":"Místo z Geoapify bylo uloženo.","geoapify-invalid":"Vybraný výsledek není platný.","geoapify-error":"Místo z Geoapify se nepodařilo uložit.","in-use":"Místo je propojené s timeline. Nejdřív ho od bodu odpojte.",coordinates:"Vyplňte obě souřadnice, nebo obě nechte prázdné.",invalid:"Zkontrolujte údaje místa.",error:"Změnu místa se nepodařilo uložit." } as const;
  const placeMessage=query.place?placeMessages[query.place as keyof typeof placeMessages]??placeMessages.error:null;
  const placeSuccess=["created","updated","removed","mapbox-saved","geoapify-saved"].includes(query.place??"");
  return <div>
    ...
    <TripPlaces canEdit={canEdit} geoapifyConfigured={Boolean(process.env.GEOAPIFY_API_KEY?.trim())} mapAccessToken={process.env.NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN?.trim()||null} places={places} tripId={tripId}/>
  </div>;
}
