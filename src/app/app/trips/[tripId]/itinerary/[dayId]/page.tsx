...
import { getItineraryDay, getTripPlaces } from "@/features/itinerary/itinerary-data";
...
const messages={created:"Bod byl přidán.",updated:"Bod byl upraven.",moved:"Pořadí timeline bylo změněno.",removed:"Bod byl odstraněn.",boundary:"Bod už je na kraji timeline.",invalid:"Zkontrolujte vyplněné údaje.",error:"Změnu se nepodařilo uložit."} as const;
...
export default async function DayPage({params,searchParams}:Props){
  ...
  const [detail,timeline,places]=await Promise.all([getTripDetail(tripId),getItineraryDay(dayId),getTripPlaces(tripId)]);
  ...
  const message=query.item?messages[query.item as keyof typeof messages]??messages.error:null; const success=["created","updated","moved","removed"].includes(query.item??"");
  ...
  return <div>
    ...
    <div className="mt-6 grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(22rem,0.82fr)]">
      <Surface depth="panel" className="p-4 sm:p-6"><div className="mb-5"><p className="text-xs font-medium tracking-[0.18em] text-primary uppercase">Program dne</p><h2 className="mt-2 text-xl font-semibold">Timeline</h2><p className="mt-1 text-sm text-muted-foreground">Aktivity, přesuny a poznámky v plánovaném pořadí.</p></div><DayTimeline canEdit={canEdit} dayId={dayId} items={timeline.items} places={places} tripId={tripId}/></Surface>
