...
import { MapboxSearchError, searchMapboxPlaces } from "@/features/places/mapbox";
...
export async function GET(request: Request) {
  ...
  try {
    const results = await searchMapboxPlaces({
      accessToken,
      countryCodes: (destinationResult.data ?? []).flatMap((destination) => destination.country_code ? [destination.country_code] : []),
      query,
    });
    return NextResponse.json({ results });
  } catch (error) {
    const status = error instanceof MapboxSearchError && error.status === 429 ? 429 : 502;
    return NextResponse.json({ error: status === 429 ? "rate_limited" : "provider_error" }, { status });
  }
}
