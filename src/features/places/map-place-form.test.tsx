...
describe("MapPlaceForm",()=>{
  ...
  it("shows read-only coordinates and cancels without submitting",()=>{vi.stubGlobal("fetch",vi.fn().mockRejectedValue(new Error()));const onCancel=vi.fn();render(<MapPlaceForm draft={{latitude:50.087123,longitude:14.407456}} onCancel={onCancel} tripId="trip"/>);expect(screen.getByLabelText("Latitude")).toHaveAttribute("readonly");expect(screen.getByLabelText("Longitude")).toHaveValue("14.407456");fireEvent.click(screen.getByRole("button",{name:"Zrušit"}));expect(onCancel).toHaveBeenCalledOnce();});
