  ...
  return <article className="min-w-0 rounded-2xl border border-border bg-background/35 p-3 sm:p-4" data-segment-key={segment.key}>
    ...
    <div id={panelId} hidden={!open} className="mt-4 grid min-w-0 gap-4">
      ...
      <div className="grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="flex flex-wrap gap-1 sm:col-span-2 lg:col-span-4"><Button className="max-sm:size-10" type="button" size="icon-sm" variant="ghost" aria-label={`Posunout segment ${index + 1} nahoru`} disabled={index === 0} onClick={() => onMove(-1)}><ArrowUp /></Button><Button className="max-sm:size-10" type="button" size="icon-sm" variant="ghost" aria-label={`Posunout segment ${index + 1} dolů`} disabled={index === total - 1} onClick={() => onMove(1)}><ArrowDown /></Button><Button className="max-sm:min-h-10" type="button" size="sm" variant="destructive" aria-label={`Odstranit segment ${index + 1}`} disabled={total === 1} onClick={onRemove}><Trash2 /> Odstranit segment</Button></div>
      </div>
    </div>
  </article>;
}
