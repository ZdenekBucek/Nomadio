...
function isTime(value: string | null) { return value === null || /^([01]\d|2[0-3]):[0-5]\d$/.test(value); }
...
export function parseItineraryItem(formData: FormData): Result {
  ...
  if (title.length < 1 || title.length > 160 || !["activity", "transport", "note"].includes(type) || !isTime(startTime) || !isTime(endTime) || (notes?.length ?? 0) > 1200 || (placeId !== null && !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(placeId))) return { error: "invalid", success: false };
