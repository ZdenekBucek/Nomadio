...
export async function getTripOverviewData(trip: { currency: string; end_date: string | null; id: string; start_date: string | null; timezone: string }, currentUserId: string) {
  ...
  return buildTripOverview({ accommodations, budgetDashboard, documents: documents.documents, itineraryDays, itineraryItems: itemResult.data ?? [], packingItems: checklist.packingItems, tasks: checklist.tasks, timezone: trip.timezone, transport, tripEnd: trip.end_date, tripId: trip.id, tripStart: trip.start_date });
}
