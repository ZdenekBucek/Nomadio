...
export function buildTripOverview(input: { accommodations: AccommodationWithPlace[]; budgetDashboard: TripBudgetDashboard; documents: DocumentWithLink[]; itineraryDays: ItineraryDayRow[]; itineraryItems: ItineraryItemRow[]; tasks: ChecklistTask[]; packingItems: ChecklistPackingItem[]; timezone: string; transport: TransportBookingWithSegments[]; tripEnd: string | null; tripId: string; tripStart: string | null }, nowDate = new Date()) {
  ...
  return { accommodation, alerts, dayItems, document: documentSummary(input.documents), finance, itinerary: { items: input.itineraryItems.length, plannedDays: plannedDays.length, totalDays: datedDays.length }, nearestPayment, nearestPaymentOverdue: Boolean(nearestPayment && input.budgetDashboard.payments.overduePayments[0]?.id === nearestPayment.id), nearestTransport, openTasks: openTasks.slice(0, 3), packing: summarizePacking(input.packingItems), selectedDay, task: summarizeTasks(input.tasks), timezone: input.timezone, upcomingAccommodation, coverage };
}
