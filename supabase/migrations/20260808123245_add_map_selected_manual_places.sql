alter table public.trip_places
  ...
  add constraint trip_places_notes_length
    check (notes is null or char_length(trim(notes)) between 1 and 1200);
...
begin
  if char_length(trim(place_name)) not between 1 and 160
    ...
    or (nullif(trim(place_notes), '') is not null and char_length(trim(place_notes)) > 1200) then
  ...
  insert into public.trip_places (
    id, trip_id, provider, provider_place_id, name, latitude, longitude, notes,
    category, category_overridden, created_by
  ) values (
    new_place_id, target_trip_id, 'manual', null, trim(place_name),
...
revoke execute on function public.create_map_selected_manual_place(
  uuid,uuid,boolean,text,public.place_category,text,double precision,double precision
) from public, anon;
